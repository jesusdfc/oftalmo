...
def plot_feature_distribution(variables_p,yvar='Atrophy_36m'):

    #check if dir exists, if not create one
    if not os.path.exists(f'{os.getcwd()}/figures/{yvar}/{variables_p}'):
        os.mkdir(f'{os.getcwd()}/figures/{yvar}/{variables_p}')
    ...
    #OPTION 3 -> CONDITIONS
    # df1 = best_conf_parameters['test_balacc'] > 0.51
    # df2 = best_conf_parameters['test_area_reject_rate'] < 0.7
    # best_conf_parameters_conditioned = best_conf_parameters[df1 & df2].copy()
    #select method
    #best_conf_parameters_conditioned = best_conf_parameters_conditioned[best_conf_parameters_conditioned.index == 'rf'].copy()
    #If there are more than 1, only 1 has to be selected
    #best_conf_parameters_conditioned = best_conf_parameters_conditioned.iloc[[0],:].copy()
    ...
    for title, best_conf_params_cond in zip(['MaxTestBalacc','MaxScore'],[best_conf_parameters_conditioned_mtb,best_conf_parameters_conditioned_mts]):
        ...
        # variables_imp_dict = variables_d.copy()
        # for x in variables_imp_dict:
        #     variables_imp_dict[x] = []
        ...
        if not no_feature_imp:
            ...
            plt.savefig(f'{os.getcwd()}/figures/{yvar}/{variables_p}/{title}_feature_imp_{yvar}_{variables_p}.pdf')


plot_feature_distribution(variables,yvar)