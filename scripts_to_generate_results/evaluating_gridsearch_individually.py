...
def dataset_comb_computation(extravars,liquids,foveolar_vars,vascular_vars,cristalin_vars,ETDRs_vars,SNPs_vars):
        ...
        return nested_gridsearch(X,y,model_type='svm')
...
def grid_models_data():
    ...
    with mlp.Pool(processes=ncores) as pool:
        df = pool.starmap(dataset_comb_computation, comb_v_l)
...
def evaluate_single_model_single_comb():

    extravars,liquids,foveolar_vars,vascular_vars,cristalin_vars,ETDRs_vars,SNPs_vars = True,False,True,True,True,True,True
    ...
    #Generate model
    model_orig = svm.SVC(C=0.1, gamma=1, kernel='sigmoid', class_weight='balanced', probability=True,
                        random_state=1)
    ...
    probs = [1-x if x>y else y for x,y in np.vstack(np.array(val_pred_proba))]
...
def trying_inconsistency_of_svm():

    from sklearn.datasets import load_digits
    digits = load_digits()

    #only 1 and 0 
    ones, zeros = np.where(digits['target']==1)[0], np.where(digits['target']==0)[0]
    ypos = np.concatenate([ones,zeros])
    y = np.array([1 for _ in ones] + [0 for _ in zeros])
    X = digits['data'][ypos.tolist(),:]
    ...
    for train_index_outer, test_index_outer in outer_cv.split(X,y):
        #
        X_outer_train, X_outer_test = X[train_index_outer,:], X[test_index_outer,:]
        y_outer_train, y_outer_test = y[train_index_outer], y[test_index_outer]
        #Compute outer balanced accuracy and confusion matrix (Validation)
        val_y.append(y_outer_test)
